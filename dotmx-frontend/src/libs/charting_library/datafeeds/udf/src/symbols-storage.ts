		...
		try {
			...
			for (; symbolIndex < symbolsCount; ++symbolIndex) {
				...
				const symbolInfo: LibrarySymbolInfo = {
					ticker: (ticker || symbolName) as string,
					name: symbolName || '',
					...
					minmove2: extractField(data, 'minmove2', symbolIndex) || extractField(data, 'minmov2', symbolIndex) || 0,
					fractional: extractField(data, 'fractional', symbolIndex) || false,
					pricescale: extractField(data, 'pricescale', symbolIndex) || 1,
					type: extractField(data, 'type', symbolIndex) || 'stock',
					session: extractField(data, 'session-regular', symbolIndex) || '24x7',
					session_holidays: extractField(data, 'session-holidays', symbolIndex) || '',
					corrections: extractField(data, 'corrections', symbolIndex) || '',
					timezone: extractField(data, 'timezone', symbolIndex) || 'Etc/UTC',
					...
					has_weekly_and_monthly: definedValueOrDefault(extractField(data, 'has-weekly-and-monthly', symbolIndex), false),
					has_empty_bars: definedValueOrDefault(extractField(data, 'has-empty-bars', symbolIndex), false),
					volume_precision: definedValueOrDefault(extractField(data, 'volume-precision', symbolIndex), 0),
					format: 'price',
				};

				this._symbolsInfo[ticker || symbolName || ''] = symbolInfo;
				if (symbolName) {
