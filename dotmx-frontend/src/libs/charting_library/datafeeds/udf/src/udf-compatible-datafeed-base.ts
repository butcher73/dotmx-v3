			...
			.then((response: Mark[] | UdfDatafeedMark) => {
				if (!Array.isArray(response)) {
					...
					for (let i = 0; i < response.id.length; ++i) {
						const borderWidth = extractField(response, 'borderWidth', i);
						const hoveredBorderWidth = extractField(response, 'hoveredBorderWidth', i);
						const imageUrl = extractField(response, 'imageUrl', i);
						const showLabelWhenImageLoaded = extractField(response, 'showLabelWhenImageLoaded', i);
						...
						if (hoveredBorderWidth !== undefined) {
							mark.hoveredBorderWidth = hoveredBorderWidth;
						}
						if (imageUrl !== undefined) {
							mark.imageUrl = imageUrl;
						}
						if (showLabelWhenImageLoaded !== undefined) {
							mark.showLabelWhenImageLoaded = showLabelWhenImageLoaded;
						}
				...
				.then((response: ResolveSymbolResponse | UdfErrorResponse) => {
					if (response.s !== undefined) {
						...
						const result: LibrarySymbolInfo = {
							...
							visible_plots_set: response.visible_plots_set ?? response['visible-plots-set'] ?? 'ohlcv',
							minmov: response.minmovement ?? response.minmov ?? 0,
							minmove2: (response.minmovement2 ?? response.minmove2) || 0,
							session: response.session ?? response['session-regular'] ?? '24x7',
							...
							has_weekly_and_monthly: response.has_weekly_and_monthly ?? response['has-weekly-and-monthly'] ?? false,
							has_empty_bars: response.has_empty_bars ?? response['has-empty-bars'] ?? false,
							volume_precision: response.volume_precision ?? response['volume-precision'] ?? 0,
