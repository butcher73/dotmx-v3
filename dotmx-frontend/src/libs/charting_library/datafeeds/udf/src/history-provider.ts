...
export class HistoryProvider {
	...
	public constructor(
		...
		if (limitedServerResponse !== undefined) {
	...
	private async _processTruncatedResponse(result: GetBarsResult, requestParams: RequestParams) {
		...
		try {
			while (this._limitedServerResponse &&
				...
				typeof requestParams.from === 'number' &&
				typeof requestParams.to === 'number' &&
				...
				if (this._limitedServerResponse.expectedOrder === 'earliestFirst') {
					if (followupResult.bars.length > 0 && result.bars.length > 0) {
						const followupFirstBar = followupResult.bars[0];
						const resultLastBar = result.bars[result.bars.length - 1];
						if (followupFirstBar && resultLastBar && followupFirstBar.time === resultLastBar.time) {
					...
					if (followupResult.bars.length > 0 && result.bars.length > 0) {
						const followupLastBar = followupResult.bars[followupResult.bars.length - 1];
						const resultFirstBar = result.bars[0];
						if (followupLastBar && resultFirstBar && followupLastBar.time === resultFirstBar.time) {
							// Datafeed shouldn't include a value exactly matching the `to` timestamp but in case it does
							// we will remove the duplicate.
	...
	private _processHistoryResponse(response: HistoryResponse | UdfErrorResponse) {
		...
		if (response.s === 'no_data') {
			meta.noData = true;
			if (response.nextTime !== undefined) {
